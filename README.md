...
| **Catálogo** | Categorías y productos con foto por categoría, precio en CLP y stock |
...
| **Estados legibles** | "En preparación" en vez de `EN_PREPARACION` en toda la UI |
| **UI moderna** | Paleta violeta/fucsia, tarjetas y pills redondeadas, navbar con degradado |
| **Cloud-native** | API Gateway con JWT Authorizer, ALB, EC2 (Docker) y ECR |
...
flowchart TB
    U([Usuario])
    SPA["SPA React<br/>(nginx · puerto 4200)"]
    GW["API Gateway HTTPS<br/>JWT Authorizer · OAuth público"]
    ALB["ALB HTTP<br/>reglas de path"]
...
| Capa | Tecnología |
|---|---|
| **Frontend** | React 18 · React Router · Vite · nginx (producción) |
| **Backend** | Java 21 · Spring Boot 3 · Spring Security · Maven |
| **Persistencia** | PostgreSQL 16 · Spring Data JPA (una DB por microservicio) |
| **Seguridad** | Spring Authorization Server (OAuth2 client) · JWT HS256 |
| **Entrega** | Docker · Docker Compose · AWS ECR / EC2 |
| **AWS** | API Gateway HTTP (v2) · Lambda (JWT Authorizer) · ALB · SSM |
...
| **ADMIN** | App role de Entra ID `ADMIN` | Todo lo del OPERADOR + CRUD de catálogo, usuarios y dashboard KPIs |
...
|---|:---:|:---:|:---:|:---:|
| `/oauth2/**` · `/login/oauth2/**` · `/api/auth/**` | Sí | — | — | — |
| `GET /api/catalog/**` | — | Sí | Sí | Sí |
| `GET /api/users/profile` | — | Sí | Sí | Sí |
| `GET/POST /api/orders` · `GET /api/orders/{id}` | — | Sí | Sí | Sí |
| `PUT /api/orders/{id}/status` | — | — | Sí | Sí |
| `POST/PUT/PATCH/DELETE /api/catalog/**` | — | — | — | Sí |
| `GET /api/users` | — | — | — | Sí |
...
| **ALB** | `http://pedidos360-alb-571995876.us-east-1.elb.amazonaws.com` — reglas de path `/api|/oauth2|/login` → BFF; resto → SPA |
| **EC2** | `i-035eb3939d3abbf02` — Docker: frontend (nginx :4200), bff (:8080), users (:8083), catalog (:8081), orders (:8082), db |
...
|---|---|:---:|
| `JWT_SECRET` | Sí | Secreto HS256 **idéntico** en users, bff y parámetro `JwtSecret` del stack |
| `JWT_EXPIRATION` | | Validez del JWT (ms). Default `86400000` (24 h) |
| `DB_USER` / `DB_PASSWORD` | | Credenciales PostgreSQL (una DB por servicio) |
| `AZURE_CLIENT_ID` / `AZURE_CLIENT_SECRET` / `AZURE_TENANT_ID` | Sí | App Registration de Entra ID |
| `AZURE_REDIRECT_URI` | Sí | `http://localhost:8080/login/oauth2/code/azure` (web: la del gateway) |
| `SPA_BASE_URL` | Sí | A dónde redirige users tras el login (`http://localhost:4200` o el ALB) |
